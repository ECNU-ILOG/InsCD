...
class Fusion(nn.Module):
    def __init__(self, student_num: int, exercise_num: int, knowledge_num: int, local_map: dict):
        ...
        self.s_from_e = local_map['s_from_e']
        self.e_from_s = local_map['e_from_s']

        super(Fusion, self).__init__()
        self.k_from_e = GraphLayer(self.k_from_e, self.knowledge_num, self.knowledge_num)
        self.e_from_k = GraphLayer(self.e_from_k, self.knowledge_num, self.knowledge_num)

        self.s_from_e = GraphLayer(self.s_from_e, self.knowledge_num, self.knowledge_num)
        self.e_from_s = GraphLayer(self.e_from_s, self.knowledge_num, self.knowledge_num)

        self.k_attn_fc1 = nn.Linear(2 * self.knowledge_num, 1, bias=True)
        self.k_attn_fc2 = nn.Linear(2 * self.knowledge_num, 1, bias=True)
        self.k_attn_fc3 = nn.Linear(2 * self.knowledge_num, 1, bias=True)

        self.e_attn_fc1 = nn.Linear(2 * self.knowledge_num, 1, bias=True)
        self.e_attn_fc2 = nn.Linear(2 * self.knowledge_num, 1, bias=True)
...
class RCD_EX(_Extractor, nn.Module):
    def __init__(self, config):
        ...
        self.if_type = config.get('if_type', None)
    ...
    def get_local_map(self, local_map):
        ...
        self.k_from_e = self.local_map['k_from_e']
        self.e_from_k = self.local_map['e_from_k']
        self.s_from_e = self.local_map['s_from_e']
        self.e_from_s = self.local_map['e_from_s']
        self.FusionLayer1 = Fusion(self.student_num, self.exercise_num, self.latent_dim, self.local_map)
        self.FusionLayer2 = Fusion(self.student_num, self.exercise_num, self.latent_dim, self.local_map)

    def __common_forward(self):
        all_stu_emb = self.__student_emb.weight
